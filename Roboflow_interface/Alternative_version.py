...
# Look at the other codes to understand how to train locally (preferred!)
...
project = rf.workspace().project("wara_bottles_detection")
model = project.version(1).model

# Define the image path
image_path = Path("WARA_bottles_detection-1/valid/images/enhanced_16_jpg.rf.e9d467213173cbfc525743616faa9380.jpg")

# Perform inference (filter out predictions with confidence below 40% and overlap below 30%)
response = model.predict(str(image_path), confidence = 0.5, overlap = 4).json()
...
print(f"The model detected {len(predictions)} objects in the image.")
